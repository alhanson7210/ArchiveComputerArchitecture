...
void armasm_print_usage (void) {
    printf("usage: ./project04 [-h hexfile] [-o objname] <filename.s>\n");
    printf("  examples: ./project04 -h foo.hex foo.s\n");
    printf("  examples: ./project04 -o bar.o bar.s\n");
}
...
int main(int argc, char **argv) {
    ...
    codegen_generate_label_pairs(&code_table, parse_tree, 0);
    codegen_stmt(&code_table, parse_tree);

    if (config.debug) {
        if (config.objoutput) {
            codegen_print_obj(&code_table);
        } 
    ...
    if (config.objoutput) {
        codegen_obj_write(&code_table, parse_tree, config.objfile);
    } 
