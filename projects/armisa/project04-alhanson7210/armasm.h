...
enum parse_opcode_enum {OC_CMP, OC_DP, OC_LS, OC_MEM, OC_MUL, OC_B, OC_BX, OC_NONE};

#define PARSE_CMP_OPS {"cmp", NULL}
#define PARSE_DP_OPS {"add", "sub", "mov", NULL}
#define PARSE_LS_OPS {"lsl", "lsr", NULL}
#define PARSE_MEM_OPS {"ldr", "ldrb", "str", "strb", NULL}
#define PARSE_MUL_OPS {"mul", NULL}
#define PARSE_B_OPS {"b", "bl", "beq", "bne", "bgt", "bge", "blt", "ble", NULL}
#define PARSE_BX_OPS {"bx", NULL}

enum parse_stmt_enum {DIR, INST, SEQ};
enum parse_inst_enum {CMP, CMPI, MOV, MOVI, DP3, DP3I, LS, LSI, MEM, MEMI, MUL, B, BX};

struct parse_node_st {
    enum parse_stmt_enum type;
    union {
        struct {
            char directive[SCAN_TOKEN_LEN];
