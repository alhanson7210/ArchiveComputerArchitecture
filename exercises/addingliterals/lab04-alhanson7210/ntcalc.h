...
#define PARSE_STRINGS {"PLUS", "MINUS", "MULT", "DIV", "LSHIFT", "BWNOT"};
#define SCAN_TOKEN_STRINGS {"TK_INTLIT", "TK_LPAREN", "TK_RPAREN", \
                            "TK_PLUS", "TK_MINUS", "TK_MULT", "TK_DIV", \
                            "TK_EOT", "TK_ANY", "TK_BITLIT", "TK_LSHIFT", "TK_BWNOT"};
...
enum parse_oper_enum {OP_PLUS, OP_MINUS, OP_MULT, OP_DIV, OP_LSHIFT, OP_BWNOT};
...
char * scan_bitlit(struct scan_table_st * tt, char * p, char * end);
...
uint32_t parse_int(const char * number, int base);
...
void eval_print(unsigned int result, int width, int base);
