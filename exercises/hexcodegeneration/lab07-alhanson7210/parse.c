...
struct parse_node_st * parse_instruction(struct parse_table_st *pt,
    ...
    tp7 = scan_table_get(st, 7);
     ...
     if ((opcode == OC_DP) && (tp1->id == TK_IDENT) && 
        ...
        np->stmt.inst.dp3.imm = 0; /* rm is a register */
        ...
        np->stmt.inst.dp3.imm = 1; /* rm is a immediate value */
        ...
        /* Parse mem instruction: (ldr|str) reg, [reg, reg] */
        ...
        np->stmt.inst.mem.imm = 1; /* offset is a register */
        ...
        /* Parse mem instruction: (ldr|str) reg, [reg, imm] */
        np->stmt.inst.type = MEM;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
        np->stmt.inst.mem.rd = parse_reg_to_int(tp1->value);
        np->stmt.inst.mem.rn = parse_reg_to_int(tp4->value);
        np->stmt.inst.mem.rm = atoi(tp6->value);
        np->stmt.inst.mem.imm = 0; /* offset is a immediate value */
        ...
        /* Parse mem instruction: (ldr|str) reg, [reg] */
        np->stmt.inst.type = MEM;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
        np->stmt.inst.mem.rd = parse_reg_to_int(tp1->value);
        np->stmt.inst.mem.rn = parse_reg_to_int(tp4->value);
        np->stmt.inst.mem.rm = 0;
        np->stmt.inst.mem.imm = 0; /* offset is a immediate value */
        ...
        /* Parse mul instruction: mul reg, reg, reg */
