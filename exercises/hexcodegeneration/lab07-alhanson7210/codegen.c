...
void codegen_mem(struct codegen_table_st *ct, struct parse_node_st *np) {
    codegen_mem_common(
        ct,
        np->stmt.inst.name,
        np->stmt.inst.mem.imm,
        np->stmt.inst.mem.rd,
        np->stmt.inst.mem.rn,
        1, /*up add np->stmt.inst.mem.ud, later */
        np->stmt.inst.mem.rm
    );
}

void codegen_mul_common(struct codegen_table_st *ct, char *name, uint32_t rd, 
    uint32_t rs, uint32_t offset) {
    const uint32_t MUL_RD_BIT = 16;
    const uint32_t MUL_RS_BIT = 8;
    const uint32_t MUL_CONST_BIT = 4;
    
    uint32_t inst = 0; /* accumulate = 0, set condition code = 0, rn = 0 | 0b0000 */
    inst = (COND_AL << COND_BIT)
        | (rd << MUL_RD_BIT)
        | (rs << MUL_RS_BIT)
